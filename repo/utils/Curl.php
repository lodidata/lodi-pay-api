...
class Curl
{
    ...
    {
        $curl = curl_init($url);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, (bool)$cacert_url );//SSL证书认证
     ...
     * @param array|null $headers 头部信息
     * @param bool $getHeader
     * @return mixed
     */
    public static function commonPost(string $url, string $cacert_url = null, string $para = '', array $headers = null, bool $getHeader = false)
    {
        $curl = curl_init($url);
        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, (bool)$cacert_url );//SSL证书认证
        curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, $cacert_url ? 2 : 0);//严格认证
